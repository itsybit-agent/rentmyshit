...
window.PendingRequestCard = {
  render(booking) {
    ...
    const meta = `<strong>${this._esc(booking.borrowerName)}</strong> \u00b7 ${dates} \u00b7 ${booking.days} days \u00b7 <strong>${booking.total} kr</strong>`;
    const email = booking.borrowerEmail
      ? `<span style="display:inline-flex;align-items:center;gap:4px;margin-left:8px;font-size:0.75rem;color:var(--muted);font-weight:400;">${emailIcon} <a href="mailto:${this._esc(booking.borrowerEmail)}" style="color:var(--muted);text-decoration:none;">${this._esc(booking.borrowerEmail)}</a></span>`
      : '';
    ...
    return `
      <div class="request-card${newClass}" data-booking-id="${booking.id}">
        ...
        <div class="request-info">
          <div class="request-item-name">${this._esc(booking.itemName)}</div>
          ...
          <div class="request-actions">
            ...
            <a href="${detailBase}?id=${encodeURIComponent(booking.item_id || '')}" class="btn btn-ghost btn-sm">View item</a>
          </div>
        </div>
      </div>`;
  },
